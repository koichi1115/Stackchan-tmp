...
4. 顔（画面）がタップされたことを Mac へ伝える（ウェイクワードの代わり。Core2 / Fire では物理ボタン A も同じ）。
...
5. 顔（画面）をタップすると `{"type":"button","name":"A"}` を送り、Mac が「はい？」と返します。ウェイクワードが通らないときの代わりです。CoreS3（StackChan）には物理ボタンが無いため、画面のどこでも構いません。Core2 / Fire では物理ボタン A でも同じです。
